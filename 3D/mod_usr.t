...
111         close(unitpar)
    ...
    subroutine my_bounds(qt,ixG^L,ixB^L,iB,w,x)
        ...
        case(3) ! bottom
            ...
            w(:,ixBmax2,:,mom(3)) = w(:,ixBmax2+1,:,mom(3))
        ...
        case(4) ! top
            ...
            w(:,ixBmin2,:,mom(3)) = w(:,ixBmin2-1,:,mom(3))
            do i=ixBmin2+1,ixBmax2
                w(:,i,:,:)      = w(:,ixBmin2,:,:)
            enddo
        case(5) ! Other bottom
            w(:,:,ixBmax3,rho_)   = w(:,:,ixBmax3+1,rho_) ! continuous
            w(:,:,ixBmax3,mom(1)) = w(:,:,ixBmax2+1,mom(1))
            w(:,:,ixBmax3,mom(2)) = w(:,:,ixBmax2+1,mom(2))
            w(:,:,ixBmax3,mom(3)) = zero
            do i=ixBmin3,ixBmax3-1
                w(:,:,i,:) = w(:,:,ixBmax3,:)
            end do
        case(6) ! Other top
            w(:,:,ixBmin3,rho_)   = w(:,:,ixBmin3-1,rho_) ! continuous
            w(:,:,ixBmin3,mom(1)) = w(:,:,ixBmin3-1,mom(1))
            w(:,:,ixBmin3,mom(2)) = w(:,:,ixBmin3-1,mom(2))
            w(:,:,ixBmin3,mom(3)) = zero
            do i=ixBmin3+1,ixBmax3
                w(:,:,i,:) = w(:,:,ixBmin3,:)
            end do
        case default
